...
if vim.fn.has 'win32' == 1 then
  vim.g.undotree_DiffCommand = 'C:\\Users\\shaim\\AppData\\Local\\nvim\\lua\\bin\\diff.exe'
end
...
-- Disable swap files because of razor
...
require('lazy').setup({
  ...
  { -- Highlight, edit, and navigate code
    ...
    opts = {
      ensure_installed = { 'bash', 'c', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'vim', 'vimdoc', 'razor' },
  ...
  },
  {
    ...
    config = function()
      ...
      end, { desc = 'Skip a new cursor by matching word/selection' })
      set({ 'n', 'x' }, '<leader>N', function()
        mc.matchAddCursor(-1)
      end, { desc = 'Add a new cursor by matching word/selection' })
      set({ 'n', 'x' }, '<leader>S', function()
